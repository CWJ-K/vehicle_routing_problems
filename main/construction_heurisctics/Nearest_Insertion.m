function [TSP_Cost]= Nearest_Insertion(k,Iter_Param,Sample_Location,Depot)

%=============================NEAREST_INSERTION_PROCEDURES================
TSP_NI = cell(1,k);                 % Create Array to Store TSP
...
% Solve the TSP using NI Procedure
for it = 1 : k
    total_route = 0;
    TSP = [Depot_Node];
    ...
    while go_sign>0
    % select the next node to insert based on Insertion Criteria = Closest
    if(go_sign>1)
        Dist_to_TSP = DM(Unvisited_Nodes{it},TSP(1,1:Tour_size-1));
