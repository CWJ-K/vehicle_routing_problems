function [TSP_Cost ]=Arbitrary_Insertion(k,Iter_Param,Sample_Location,Depot)
%=============================ARBITRARY_INSERTION_PROCEDURES================
TSP_AI = cell(1,k);                 % Create Array to Store TSP
...
for it = 1 : k
    ...
    while go_sign>0
    % select the next node randomly to insert based on Insertion Criteria =
    % Closest
    if(go_sign>1)
        nodeindex=randi(length(Unvisited_Nodes{it}));
