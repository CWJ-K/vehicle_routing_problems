...
StoreTable = {'No', 'Problem','Best Value','Farthest Insertion Cost', 'CLS inter Cost','TB Cost','Time (in second)'};
for length = 1  : length(Filelist)
    newData = load(FileName{length}, 'Demand','Sample_Location','Capacity','Depot','Best_Value');
...
%===============================Tabu====================================================
    ...
    for cl=1:k
        BestImp=-1;
        TSP = TSP_New_Route{cl};
        Nb_Nodes= size(TSP,2);
        TSPSize=size(TSP,2);
        
        Cost_all = TSP_New_Cost{cl};
        Cost_Input = Cost_all;
...
StoreTable{length+1,5} = NewTB_Cost;
StoreTable{length+1,6} = waktu;
sprintf ("Finished %d", length)
end



xlsFile = 'output_TB_Intra.xlsx';
sheetName='TB';
data = StoreTable;
