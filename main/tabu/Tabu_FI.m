...
StoreTable = {'No', 'Problem','Farthest Insertion Cost', 'CLS Cost','CLS_Introute_Cost','TB Cost','Time (in second)'};
...
CLS_IR_Cost = 0 ;
for it = 1:k
  inscost = TSP_IR_Cost{it};
  CLS_IR_Cost = CLS_IR_Cost + inscost;
end
...
StoreTable{length+1,5} = CLS_IR_Cost;
StoreTable{length+1,6} = NewTB_Cost;
StoreTable{length+1,7} = waktu;
sprintf ("Finished %d", length)
end



xlsFile = 'output_TB_list.xlsx';
sheetName='TB';
data = StoreTable;
